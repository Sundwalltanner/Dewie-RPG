...
class Events extends BaseHelper {
  ...
  async skillEvent(loadedPlayer, events, globalMultiplier) {
    const updatedPlayer = Object.assign({}, loadedPlayer);
    try {
      const skillDice = await this.randomBetween(1, 4);
      switch (skillDice) {
        // Woodcutting
        case 1:
          return this.SkillEvents.woodcuttingEvent(updatedPlayer, (globalMultiplier + updatedPlayer.personalMultiplier));

        // Mining
        case 2:
          return this.SkillEvents.miningEvent(updatedPlayer, (globalMultiplier + updatedPlayer.personalMultiplier));

        // Fishing
        case 3:
          return this.SkillEvents.fishingEvent(updatedPlayer, (globalMultiplier + updatedPlayer.personalMultiplier));

        // Herbalism
        case 4:
          return this.SkillEvents.herbalismEvent(updatedPlayer, (globalMultiplier + updatedPlayer.personalMultiplier));
      }

/*
      // Mining
      if (skillDice == 1) {
        return this.SkillEvents.miningEvent(updatedPlayer);
      }

      // Fishing
      if (skillDice == 2) {
        return this.SkillEvents.fishingEvent(updatedPlayer);
      }

      if (this.MapManager.getTowns().includes(updatedPlayer.map.name)) {
        // Cooking
        if (skillDice == 0) {
          return this.SkillEvents.cookingEvent(updatedPlayer);
        }

        // Smithing
        if (skillDice == 1) {
          return this.SkillEvents.SmithingEvent(updatedPlayer);
        }

        // Crafting
        if (skillDice == 2) {
          return this.SkillEvents.craftingEvent(updatedPlayer);
        }
      }
      */
    } catch (err) {
