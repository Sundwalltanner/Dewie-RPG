  ...
  async steal(stealingPlayer, victimPlayer) {
    ...
    try {
      ...
      if (luckStealChance > (90 - canSteal)) {
        const luckItem = await this.randomBetween(0, 7);
        const itemKeys = [enumHelper.equipment.types.helmet.position, 
                          enumHelper.equipment.types.bodyArmour.position, 
                          enumHelper.equipment.types.gloves.position,
                          enumHelper.equipment.types.weapon.position,
                          enumHelper.equipment.types.amulet.position,
                          enumHelper.equipment.types.belt.position,
                          enumHelper.equipment.types.ringRight.position,
                          enumHelper.equipment.types.ringLeft.position];

        if (![enumHelper.equipment.empty.helmet.name, 
              enumHelper.equipment.empty.bodyArmour.name, 
              enumHelper.equipment.empty.gloves.name,
              enumHelper.equipment.empty.weapon.name,
              enumHelper.equipment.empty.amulet.name,
              enumHelper.equipment.empty.belt.name,
              enumHelper.equipment.empty.ringRight.name,
              enumHelper.equipment.empty.ringLeft.name].includes(victimPlayer.equipment[itemKeys[luckItem]].name)) {
          let stolenEquip;
