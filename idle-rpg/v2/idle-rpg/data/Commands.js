...
class Commands extends aggregation(BaseGame, BaseHelper) {
  ...
  async joinLottery(params) {
    ...
    if (player.gold.current >= 100)
    {
      ...
      const lotteryChannel = await Bot.guilds.find(guild => guild.id === player.guildId).channels.find(channel => channel.name === 'lottery' && channel.type === 'text' && channel.parent && channel.parent.name === 'Dewie-RPG');
      if (lotteryChannel) {
      ...
      return author.send(`You do not have enough gold! The lottery costs 100 gold to enter. You're lacking ${100 - player.gold.current} gold.`);
    }
...
${rankString}\`\`\``);
  ...
  async modifyServerPrefix(params) {
    ...
    try {
      ...
      const faqChannel = await server.channels.find(channel => channel.name === 'faq' && channel.type === 'text' && channel.parent.name === 'Dewie-RPG');
