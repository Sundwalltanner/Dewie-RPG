...
class Game extends aggregation(BaseGame, BaseHelper) {
    ...
    try {
      ...
      if (!loadedPlayer) {
        ...
        return await this.updatePlayer({
          ...
          msg: [`${this.generatePlayerName(newPlayer, true)} was born in \`${newPlayer.map.name}\`! Welcome to the world of Dewie-RPG!`],
          pm: ['You were born.']
        });
      }
      /*
      ...
      loadedPlayer.skills.total.level = loadedPlayer.skills.mining.level + loadedPlayer.skills.fishing.level + loadedPlayer.skills.woodcutting.level + loadedPlayer.skills.herbalism.level;
      loadedPlayer.skills.total.experience = loadedPlayer.skills.mining.experience.total + loadedPlayer.skills.fishing.experience.total + loadedPlayer.skills.woodcutting.experience.total + loadedPlayer.skills.herbalism.experience.total;
  ...
  async selectEvent(loadedGuildConfig, loadedPlayer, onlinePlayers) {
    try {
      const randomEvent = await this.randomBetween(0, 3);
      switch (randomEvent) {
        case 0:
        ...
        case 3:
          return this.Events.skillEvent(loadedPlayer, loadedGuildConfig.events, loadedGuildConfig.multiplier);
      }
    } catch (err) {
      errorLog.error(err);
    }
