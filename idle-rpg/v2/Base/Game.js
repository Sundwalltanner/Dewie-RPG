  ...
  async checkHealth(Database, MapClass, playerObj, attackerObj, eventMsg, eventLog, otherPlayerPmMsg) {
    ...
    try {
      if (updatedPlayer.health <= 0) {
        ...
        const deathMsg = await this.randomDeathEventMessage();
        eventMsg.push(this.setImportantMessage(`${updatedPlayer.name}${updatedPlayer.titles.current !== 'None' ? ` the ${updatedPlayer.titles.current}` : ''} died${expLoss === 0 ? '' : ` and lost ${expLoss} exp`}${goldLoss === 0 ? '' : ` and lost ${goldLoss} gold`}! ${deathMsg.eventMsg}`));
        eventLog.push(`You died${expLoss === 0 ? '' : ` and lost ${expLoss} exp`}${goldLoss === 0 ? '' : ` and lost ${goldLoss} gold`}. ${deathMsg.eventLog}`);
        let bountyEventLog;
    ...
    try {
      if (updatedPlayer.experience.current >= updatedPlayer.level * 15) {
        ...
        const playerStats = await Object.keys(updatedPlayer.stats).map((key) => {
          if (['str', 'dex', 'end', 'int'].includes(key)) {
            return {
              key,
        ...
        const applicableClassList = enumHelper.classes[playerStats[0].key].filter(playerClass => playerClass.level <= updatedPlayer.stats[playerStats[0].key]);
        updatedPlayer.class = applicableClassList[applicableClassList.length - 1].name;
