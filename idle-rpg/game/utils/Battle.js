...
class Battle extends BaseHelper {
  ...
  inventoryTurn(attacker, defender, battleStats, attackerDamage, defenderDamage) {
    return new Promise((resolve) => {
      ...
      if (initiative.name === attacker.name) {
        ...
        const attackerPotions = attacker.inventory.consumables.filter(item => item.name.includes('Health Potion'));
        if (attacker.inventory.consumables.length > 0 && attackerPotions.length > 0) {
          ...
          attacker.inventory.consumables = attacker.inventory.consumables.splice(attacker.inventory.consumables.indexOf(potion), 1);
        ...
        const defenderPotions = defender.inventory.consumables.filter(item => item.name.includes('Health Potion'));
        if (defender.inventory.consumables.length > 0 && defenderPotions.length > 0) {
          ...
          defender.inventory.consumables = defender.inventory.consumables.splice(defender.inventory.consumables.indexOf(potion), 1);
      ...
      case 'melee':
        attackPower = this.isMonster(player)
          ...
          : (this.sumPlayerTotalStrength(player) 
            ...
            + (player.equipment.amulet.power)
            + (player.equipment.ringRight.power)
            + (player.equipment.ringLeft.power))
  ...
  calculateDefense(player) {
    const physicalDefensePower = this.isMonster(player)
      ? (player.stats.end + player.equipment.bodyArmour.power + player.power) + ((player.stats.dex / 2) + (player.stats.luk / 2))
      : (this.sumPlayerTotalEndurance(player)
        + (player.equipment.helmet.power / 2)
        + (player.equipment.bodyArmour.power / 2)
        + (player.equipment.gloves.power / 2)
        + (player.equipment.belt.power / 2))
      + (this.sumPlayerTotalDexterity(player) + (this.sumPlayerTotalLuck(player) / 2));
    const magicDefensePower = this.isMonster(player)
      ? (player.stats.int + player.equipment.bodyArmour.power + player.power) + ((player.stats.dex / 2) + (player.stats.luk / 2))
      : (this.sumPlayerTotalIntelligence(player)
        + (player.equipment.helmet.power / 2)
        + (player.equipment.bodyArmour.power / 2)
        + (player.equipment.gloves.power / 2)
